...
describe('AppNav', () => {
  ...
  it('shows the in-flight badge count on Reconciliation when fetchAwaitingReview returns batches', async () => {
    ...
    const w = await mountNav('/history') // Mount on a different route so we can see the tabs clearly
    await flushPromises()
    expect(w.text()).toContain('1')   // badge value present in rendered text
    const reconLink = w.findAll('a').find(l => l.text().includes('Reconciliation'))
    expect(reconLink!.text()).toContain('1')
  })
})
